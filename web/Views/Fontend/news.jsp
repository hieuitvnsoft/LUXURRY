...
<html>
    ...
    <div class="centered page"> 
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            <c:forEach items="${listTypeNews}" var="list" varStatus="x">
                <li class="nav-item">
                    <a class="nav-link <c:if test="${list.typeNewId==1}">active</c:if>" id="home-tab" data-toggle="tab" href="#${list.typeNewId}bbb" role="tab" aria-controls="${list.typeNewName}bbb" aria-selected="true">${list.typeNewName}</a>
                    </li>
            </c:forEach>

        </ul>
        <div class="tab-content">
            <c:forEach items="${listTypeNews}" var="list1">  
                <div class="tab-pane <c:if test="${list1.typeNewId==1}">active</c:if>" id="${list1.typeNewId}bbb" role="tabpanel" aria-labelledby="${list1.typeNewId}-tab">

                    <c:forEach items="${listNews}" var="newss">
                        <c:if   test="${list1.typeNewId==newss.typeId.typeNewId}">
                            <a href="${pageContext.request.contextPath}/newP.htm?id=${newss.id}">
                                <div class="row">
                                    <div class="col-md-3"><img style="width: 100%" src="/${newss.newImage}"/></div>
                                    <div class="col-md-9">
                                        <h4>${newss.title}</h4>
...
<div style="border: 1px solid #000" class=""></div>
